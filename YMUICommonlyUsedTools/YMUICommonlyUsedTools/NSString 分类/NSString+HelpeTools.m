...
"xy";
@implementation NSString (HelpeTools)
#pragma mark  dic 转json
...
+ (NSString *)chinaesExchangeToFirstPingyin:(NSString *)chinaesString
{
    ...
    BOOL isJixu  = YES;
    while (isJixu) {
        if (chinaesString.length > 1) {
            isJixu = YES;
            [arr addObject:[chinaesString substringToIndex:1]];
            chinaesString = [chinaesString substringFromIndex:1];
        }else{
            isJixu = NO;
            [arr addObject:chinaesString];
        }
    }
    NSString *sumString ;  //组合字符串
    int i = 0 ;
    for (NSString *s in arr) {
        if (i == 0) {
            sumString = [s getFirstLetter];
            i ++ ;
            continue ;
        }
        sumString = [NSString stringWithFormat:@"%@%@",sumString,[s getFirstLetter]];
    }
    sumString = [sumString lowercaseString];
...
+ (NSDate *)getCurrenTimeDate:(NSString *)cuuerntime
{
    ...
    [formater setDateFormat:@"yyyy 年 MM 月 dd 日"];
    NSDate* date = [formater dateFromString:cuuerntime];
    return date ;
}
...
#pragma mark 判断是否是纯中文
- (BOOL)isChinese
{
    NSString *match = @"(^[\u4e00-\u9fa5]+$)"; NSPredicate *predicate = [NSPredicate predicateWithFormat:@"SELF matches %@", match];
...
+ (NSMutableAttributedString *)exchangeStringToAttributedString:(NSString *)originalString AndRange:(NSRange)substringWithRange AndLabelColor:(UIColor *)color AndTextFont:(NSInteger)Font
{
    ...
    //字体大小
    if (Font) {
        [attributedString addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:Font] range:substringWithRange];
    } else {
        [attributedString addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:10] range:substringWithRange];
    }
    
    //颜色
    if (color) {
        [attributedString addAttribute:NSForegroundColorAttributeName value:color range:substringWithRange];
    } else {
        [attributedString addAttribute:NSForegroundColorAttributeName value:[UIColor grayColor] range:substringWithRange];
    }
